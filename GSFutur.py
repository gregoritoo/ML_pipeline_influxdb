...
class GSFutur():
    ...
    def fit_with_UI(self,df,verbose=0, nb_features=1, nb_epochs=300, nb_batch=100, nb_layers=50,
    ...
    def predict(self, steps=1, frame=True):
        print(self.shift)
        steps=steps+self.shift
        prediction, lower, upper = self.make_prediction(steps)
        if frame:
            return prediction[0][int(self.shift/2):], lower[0][int(self.shift/2):], upper[0][int(self.shift/2):]
        else:
            return prediction[0][int(self.shift/2):]
    ...
    def train_model(self, model, x_train, y_train, name):
        ...
        if name == "trend":
            ...
            es = EarlyStopping(monitor='mse', mode='min', min_delta=0.1, patience=200)
            ...
            x_train = x_train.reshape(x_train.shape[0], 1, x_train.shape[1])
            es = EarlyStopping(monitor='mse', mode='min', min_delta=0.001, patience=200)
            hist = model.fit(x_train, y_train, epochs=nb_epochs, batch_size=self.nb_batch, verbose=self.verbose,
                             callbacks=[es])
            i = 0
            self.model_save(model, name)
        else:
            es = EarlyStoppingByUnderVal(monitor="mse", value=0.005, verbose=self.verbose)
            x_train = x_train.reshape(x_train.shape[0], 1, x_train.shape[1])
